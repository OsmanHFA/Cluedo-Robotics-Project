...
startGazebo() {
    ...
    countdown 'Starting Gazebo...' '00:00:20'
}

loadLocalisation() {
    localisation="roslaunch $PROJECT_PATH/launch/simulated_localisation.launch map_file:=${1} 2> >(grep -v TF_REPEATED_DATA buffer_core)"
    if [ $DEBUG == 0 ]; then
...
launchRviz() {
    rviz="roslaunch turtlebot_rviz_launchers view_navigation.launch 2> >(grep -v TF_REPEATED_DATA buffer_core)"
    if [ $DEBUG == 0 ]; then
